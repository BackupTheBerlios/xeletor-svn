...
begin
  FillByte(KernelName{%H-},SizeOf(KernelName),0);
  fpUname(KernelName);
...
begin
  inotify_add_watch := -1;
  if IsGoodKernelVersion then
    inotify_add_watch := do_syscall(syscall_nr_inotify_add_watch, TSysParam(__fd), {%H-}TSysParam(__name), TSysParam(__mask));
end;
